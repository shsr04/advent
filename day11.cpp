...
int main(int argc, char **argv) {
    ...
    while (!m.halted_) {
        ...
        auto new_colour = r.front(), rotation = r.back();
        // cout << robot_pos << " at " << robot_dir << "\n";
        // cout << "[" << colour << "->" << (is_fresh ? '!' : ' ') << new_colour
        //      << "], turn " << (rotation == 1 ? "right" : "left") << "\n";
        colours[robot_pos] = new_colour;
        if (is_fresh)
            n_coloured++;
        if (rotation == 1) // right
            robot_dir -= 90;
