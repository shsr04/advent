...
static int countZeroPasses(int __metac_in_base, int __metac_in_delta);
  ...
  int zeroPasses = 0;
  {
    ...
    while (fgets(line, sizeof(line), stdin) != NULL) {
      ...
      if (metac_streq(direction, "L")) {
        zeroPasses = (zeroPasses + countZeroPasses(dial, (-amount)));
...
static int countZeroPasses(int __metac_in_base, int __metac_in_delta) {
  int __metac_line_no = 0;
  char __metac_err[160];
  const int base = metac_wrap_range(__metac_in_base, 0, 99);
  const int delta = __metac_in_delta;
  const int isNegative = (delta < 0);
  int current = metac_wrap_range(base, 0, 99);
  int steps = 0;
  {
    if (isNegative) {
      steps = (-delta);
  ...
  while ((steps > 0)) {
    if (isNegative) {
      current = metac_wrap_range((current - 1), 0, 99);
    }
    else {
      current = metac_wrap_range((current + 1), 0, 99);
    }
