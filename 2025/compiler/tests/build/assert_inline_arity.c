...
static ResultNumber solve(void) {
  ...
  ResultStringList __metac_split1 = metac_split_string("10-12", "-");
  ...
  if (!((((int64_t)__metac_assert_list9.count) == 2))) {
    return err_number("Invalid range expression", __metac_line_no, "");
  }
  ...
  int64_t total = 0;
  int64_t __metac_seq_start11 = start;
  int64_t __metac_seq_end12 = end;
  if (__metac_seq_start11 <= __metac_seq_end12) {
    for (int64_t __metac_seq_i13 = __metac_seq_start11; __metac_seq_i13 <= __metac_seq_end12; __metac_seq_i13++) {
      const int64_t x = __metac_seq_i13;
      total = (total + x);
    }
  } else {
    for (int64_t __metac_seq_i13 = __metac_seq_start11; __metac_seq_i13 >= __metac_seq_end12; __metac_seq_i13--) {
