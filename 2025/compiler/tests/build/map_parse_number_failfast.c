...
static ResultNumber solve(void) {
  ...
  ResultStringList __metac_split0 = metac_split_string(metac_read_all_stdin(), ",");
  if (__metac_split0.is_error) {
    return err_number(__metac_split0.message, __metac_line_no, "");
  }
  StringList ranges = __metac_split0.value;
  int64_t total = 0;
  for (size_t __metac_i1 = 0; __metac_i1 < ranges.count; __metac_i1++) {
    const char *range = ranges.items[__metac_i1];
    ResultStringList __metac_split2 = metac_split_string(range, "-");
    if (__metac_split2.is_error) {
      return err_number(__metac_split2.message, __metac_line_no, "");
    }
    StringList parts = __metac_split2.value;
    StringList __metac_map_src3 = parts;
    ...
    NumberList bounds;
    bounds.count = __metac_map_count4;
    bounds.items = __metac_map_items6;
    if ((((int64_t)bounds.count) == 2)) {
      NumberList __metac_list9 = bounds;
      const int64_t start = __metac_list9.items[0];
      const int64_t end = __metac_list9.items[1];
      int64_t __metac_seq_start10 = start;
      int64_t __metac_seq_end11 = end;
      if (__metac_seq_start10 <= __metac_seq_end11) {
        for (int64_t __metac_seq_i12 = __metac_seq_start10; __metac_seq_i12 <= __metac_seq_end11; __metac_seq_i12++) {
          const int64_t x = __metac_seq_i12;
          total = (total + x);
        }
      } else {
        for (int64_t __metac_seq_i12 = __metac_seq_start10; __metac_seq_i12 >= __metac_seq_end11; __metac_seq_i12--) {
      ...
      return err_number(metac_fmt("Invalid range expression: %s", range), __metac_line_no, "");
    }
  }
