...
static ResultNumber solve(void) {
  ...
  char range[256];
  metac_copy_str(range, sizeof(range), "2-4");
  ResultStringList __metac_split1 = metac_split_string(range, "-");
  ...
  StringList __metac_map_src2 = __metac_chain0;
  size_t __metac_map_count3 = __metac_map_src2.count;
  int64_t *__metac_map_items5 = (int64_t *)calloc(__metac_map_count3 == 0 ? 1 : __metac_map_count3, sizeof(int64_t));
  if (__metac_map_items5 == NULL) {
    return err_number("out of memory in map", __metac_line_no, "");
  }
  for (size_t __metac_map_i4 = 0; __metac_map_i4 < __metac_map_count3; __metac_map_i4++) {
    int64_t __metac_map_num6 = 0;
    if (!metac_parse_int(__metac_map_src2.items[__metac_map_i4], &__metac_map_num6)) {
      return err_number("Invalid number", __metac_line_no, __metac_map_src2.items[__metac_map_i4]);
    }
    __metac_map_items5[__metac_map_i4] = __metac_map_num6;
  }
  NumberList bounds;
  bounds.count = __metac_map_count3;
  bounds.items = __metac_map_items5;
  if ((((int64_t)bounds.count) != 2)) {
    return err_number(metac_fmt("Invalid range: %s", range), __metac_line_no, "");
  }
  NumberList __metac_list8 = bounds;
  const int64_t start = __metac_list8.items[0];
  const int64_t end = __metac_list8.items[1];
  int64_t total = 0;
  int64_t __metac_seq_start9 = start;
  int64_t __metac_seq_end10 = end;
  if (__metac_seq_start9 <= __metac_seq_end10) {
    for (int64_t __metac_seq_i11 = __metac_seq_start9; __metac_seq_i11 <= __metac_seq_end10; __metac_seq_i11++) {
      const int64_t x = __metac_seq_i11;
      total = (total + x);
    }
  } else {
    for (int64_t __metac_seq_i11 = __metac_seq_start9; __metac_seq_i11 >= __metac_seq_end10; __metac_seq_i11--) {
