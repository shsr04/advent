...
static int isSmall(int64_t __metac_in_v);
static ResultNumber solve(void);
static int64_t stepsToZero(int64_t __metac_in_n);


static int isSmall(int64_t __metac_in_v) {
  int __metac_line_no = 0;
  char __metac_err[160];
  const int64_t v = __metac_in_v;
  return (v < 5);
  return 0;
}
...
static ResultNumber solve(void) {
  int __metac_line_no = 0;
  char __metac_err[160];
  const int64_t value = stepsToZero(3);
  if (isSmall(value)) {
    return ok_number(value);
  }
  else {
    return ok_number(0);
  }
