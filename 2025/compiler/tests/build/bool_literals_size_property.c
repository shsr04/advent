...
static int hasRepeatedHalf(const char * __metac_in_input);
...
static int hasRepeatedHalf(const char * __metac_in_input) {
  ...
  StringList chunks = metac_chunk_string(input, (metac_strlen(input) / 2));
  if ((((int64_t)chunks.count) != 2)) {
    return 0;
  }
  StringList __metac_list0 = chunks;
...
static ResultNumber solve(void) {
  ...
  char __metac_err[160];
  int64_t score = 0;
  if (hasRepeatedHalf("1212")) {
    score = (score + 1);
  }
  if (hasRepeatedHalf("12345")) {
    score = (score + 10);
  }
