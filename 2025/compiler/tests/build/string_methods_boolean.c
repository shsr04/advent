...
static int hasRepeatingDigits(const char * __metac_in_input);
...
static int hasRepeatingDigits(const char * __metac_in_input) {
  ...
  const char *input = __metac_in_input;
  if ((((int64_t)metac_chunk_string(input, (metac_strlen(input) / 2)).count) != 2)) {
    return (1 == 0);
  }
  StringList __metac_list0 = metac_chunk_string(input, (metac_strlen(input) / 2));
  const char *head = __metac_list0.items[0];
  const char *tail = __metac_list0.items[1];
  return metac_streq(head, tail);
  return 0;
}
...
static ResultNumber solve(void) {
  ...
  char __metac_err[160];
  int64_t score = 0;
  if (hasRepeatingDigits("123123")) {
    score = (score + 1);
  }
  if (hasRepeatingDigits("1212")) {
    score = (score + 1);
  }
  if (hasRepeatingDigits("12345")) {
    score = (score + 10);
  }
  if (hasRepeatingDigits("111")) {
    score = (score + 100);
  }
  return ok_number(score);
