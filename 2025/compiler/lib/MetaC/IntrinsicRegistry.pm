...
sub method_base_specs {
    return {
        ...
        last => { arity => 0, expr_callable => 1, fallibility => 'never', receivers => { string_list => 1, number_list => 1, number_list_list => 1, bool_list => 1, indexed_number_list => 1 } },
        ...
        map => { arity => 1, expr_callable => 0, fallibility => 'mapper', receivers => { string_list => 1, number_list => 1, bool_list => 1 } },
        ...
        insert => { arity => 2, expr_callable => 1, fallibility => 'conditional', receivers => { matrix_any => 1, number_list => 1, number_list_list => 1, string_list => 1, bool_list => 1 } },
...
sub method_codegen_templates {
    return {
        ...
        last => {
            string_list         => { arity => 0, result_type => 'string', expr_template => 'metac_string_list_last_or_die(%RECV%)' },
            number_list         => { arity => 0, result_type => 'number', expr_template => 'metac_number_list_last_or_die(%RECV%)' },
            number_list_list    => { arity => 0, result_type => 'number_list', expr_template => 'metac_number_list_list_last_or_die(%RECV%)' },
            bool_list           => { arity => 0, result_type => 'bool', expr_template => 'metac_bool_list_last_or_die(%RECV%)' },
            indexed_number_list => { arity => 0, result_type => 'indexed_number', expr_template => 'metac_indexed_number_list_last_or_die(%RECV%)' },
        },
        sort => {
